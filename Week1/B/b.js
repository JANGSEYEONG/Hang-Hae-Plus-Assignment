...
class HardWork {
  ...
  do() {
    ...
    // 마이크로 테스크로 일을 처리하게 만들어 우선순위를 가져가야하나,
    // 마이크로 테스크에 한번에 다 넣어버리면 렌더링이 되지 않기 때문에, 조금씩 나눠서 마이크로 큐에 넣거나
    // 한 묶음이 끝나면 setInterval로 중간에 한번 매크로 큐로 보내줘야함
    const taskSize = 100; //100개씩 처리
    const taskLength = this._tasks.length;
    let start = 0;
    let doTask = setInterval(()=>{
      // 일을 다 했으면 interval을 멈춘다.
      if(start > taskLength){
